...
export class GitHubService {
  ...
  async pushToGitHub(
    ...
    try {
      ...
      console.log(ui.format.info(`[GitHubService] ${progressMessage}`));
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(ui.format.success(`[GitHubService] ${progressMessage}`));
        } else if (response.status === 404) {
          progressMessage = 'Creating GitHub repository...';
          console.log(ui.format.warn(`[GitHubService] ${progressMessage}`));
      ...
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        console.warn(
          ui.format.warn(`[GitHubService] Could not determine push status, attempting push anyway: ${errorMessage}`),
        );
      }
      ...
      try {
        if (existsSync(indexLockPath)) {
          console.log(ui.format.warn('[GitHubService] Removing stale git index lock...'));
      ...
      console.log(ui.format.success(`[GitHubService] ${successMessage}`));
      return {
        success: true,
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(ui.format.error(`[GitHubService] Failed to push to GitHub: ${errorMessage}`));
      throw error;
    }
  }
}
