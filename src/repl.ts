...
import { addFromDef, addSchemaFromDef, addRelationshipFromDef, addWorkflowFromDef } from 'agentlang/out/runtime/loader.js';
...
async function processAgentlang(code: string): Promise<string> {
  ...
  if (!currentModule && replState?.appSpec?.name) {
...
function createReplHelpers() {
  ...
  const al = function(strings: any, ...values: any[]): Promise<string> {
  ...
  const e = function(name: string, definition?: any): Promise<string> {
  ...
  // Record function
  const r = function(name: string, definition?: any): Promise<string> {
    if (typeof name === "string" && typeof definition === "object") {
      const fields = Object.entries(definition)
        .map(([key, type]) => `  ${key} ${type}`)
        .join("\\n");
  ...
  // Event function
  const ev = function(name: string, definition?: any): Promise<string> {
    if (typeof name === "string" && typeof definition === "object") {
      const fields = Object.entries(definition)
        .map(([key, type]) => `  ${key} ${type}`)
        .join("\\n");
  ...
  const inst = function(entityName: string, attributes: any): Instance {
...
async function restartRepl(): Promise<void> {
  ...
  try {
    ...
    console.error(chalk.red(`❌ Failed to restart: ${error}`));
  } finally {
...
function cleanup(): void {
  if (replState) {
    ...
    if (replState.watcher) {
      replState.watcher.close();
    }
  }
}

// Main REPL function
export async function startRepl(appDir: string = '.', options: ReplOptions = {}): Promise<void> {
  ...
  replState = {
    ...
    rl: readline.createInterface({
      ...
      completer: (line: string) => {
        const completions = [
          'help', '?', 'exit', 'quit', 'clear', 'restart',
          'al`', 'al(', 'e(', 'r(', 'ev(', 'rel(', 'w(', 'inst(',
          'm.list()', 'm.active()', 'm.get(', 'm.add(', 'm.remove(',
          'inspect.modules()', 'inspect.entities()', 'inspect.events()', 'inspect.relationships(',
          'utils.help()', 'utils.clear()', 'utils.restart()', 'utils.exit()',
          'addEntity(', 'removeEntity(', 'getEntity(',
          'addRecord(', 'removeRecord(', 'getRecord(',
          'addEvent(', 'removeEvent(', 'getEvent(',
          'addRelationship(', 'removeRelationship(', 'getRelationship(',
          'addWorkflow(', 'removeWorkflow(', 'getWorkflow(',
        ];
  ...
  try {
    ...
    replState.rl.on('line', async (input: string) => {
